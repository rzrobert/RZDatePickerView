...
- (void)showDatePickerWithType:(RZDateStyle)type {
    RZDatePickerView *datepicker = [[RZDatePickerView alloc] initWithCurrentDate:[NSDate date:@"2017-1-15 11:11" WithFormat:@"yyyy-MM-dd HH:mm"] CompleteBlock:^(NSDate *startDate) {
        NSLog(@"======%@",startDate);
        self.timeLabel.text = [startDate stringWithFormat:@"yyyy-MM-dd HH:mm"];
    }];
    
    datepicker.datePickerStyle = self.datePickerType;
